# ロードマップ / 実装チェックリスト

[`DESIGN.md`](./DESIGN.md) §14 のフェーズを、実装タスクへ落とし込んだもの。
チェックは進捗に応じて更新する。

---

## フェーズ0 — 設計（本書群）✅ 進行中
- [x] 現状分析（ハードコード箇所・既知の不具合の洗い出し）
- [x] アーキテクチャ方針（静的コア＋動的レイヤ）
- [x] データモデル・設定スキーマの定義
- [x] データ投入方式の決定（ファイル方式を推奨採用）
- [ ] 時限時刻の正値確認（Python版/JS版どちらが正か）
- [ ] フロント統合の方針A/B 決定
- [ ] 独自ドメイン/ルーティング方式（パス/サブドメイン）決定

## フェーズ1 — テナント化MVP（1コードで複数校・NUST移行込み）
- [ ] `schools/index.json` と `schools/<slug>/config.yml` のローダ
- [ ] 検索ロジックの汎用化（校舎別 if 分岐の撤去、差集合を学校非依存に）
- [ ] 時限・学期・曜日・校舎・色・学校名を config から注入（ハードコード排除）
- [ ] 時限時刻の単一ソース化（サーバ→テンプレ→JS へ一方向供給）
- [ ] API を `/api/<slug>/…` 化、入力検証を学校別許可集合に
- [ ] 予約・報告DBへ `school` 列追加＋複合インデックス、localStorage キー拡張
- [ ] 全国トップページ（学校検索/一覧）
- [ ] NUST 移行：DB→CSV エクスポート、config 作成、**旧実装との回帰一致確認**

## フェーズ2 — 静的コア化＋貢献導線
- [ ] 空き計算のクライアント化（学校別 `schedule.<slug>.json` を配信）
- [ ] CDN/静的ホスティング配信＋キャッシュ・バージョニング
- [ ] `scripts/validate.py`（フォーマット検証）／`scripts/build.py`（JSON・index 生成）
- [ ] CI で検証自動化（PR時にデータをチェック）
- [ ] `CONTRIBUTING.md` ／ Issue テンプレ「学校追加」／フォーム導線
- [ ] フロント2系統（app.py テンプレ / index.html）の統合

## フェーズ3 — スケール運用（数十〜数百校）
- [ ] DBファイル分割 or Postgres 移行（`school` で水平分割可能に）
- [ ] 解析ダッシュボードの学校横断対応
- [ ] （任意）データ編集の管理UI

## フェーズ4 — 全国グロース
- [ ] 都道府県/地域ディレクトリ
- [ ] データ鮮度管理（学期改訂検知・再ビルド運用）
- [ ] 各校メンテナ制度・運営ドキュメント
- [ ] SNS自動化の学校別テンプレ化（任意機能として分離）

---

### 完了の定義（フェーズ1の受け入れ基準・例）
- 新しい学校を **コードを一切変更せず** `schools/<slug>/` の追加だけで公開できる。
- NUST の検索結果が **旧 nust-room-search と一致**（曜日×時限×校舎で突合）。
- 予約・報告が学校間で衝突しない。
